...
public class ShapeRightSeven extends Shape implements InterfaceShapeMotion{
	...
	GamePanel gamePanel;
	public ShapeRightSeven(GamePanel gamePanel) {
		...
		shapePositionPoint[2] = blockPoint[gamePanel.xPointCount / 2 + 0][1];
		shapePositionPoint[3] = blockPoint[gamePanel.xPointCount / 2 + 1][1];
	...
	public void changeShape() {
		...
		switch (shape) {
			...
			case 0:
			{   // 由向上形状变换为向右形状
				if(y + 2 < gamePanel.yPointCount && x + 2 < gamePanel.xPointCount) {   // 判断形状变换后是否超出游戏区域边界
					
					block[0] = blockPoint[x + 2][y + 0];
			...
			case 1:
			{   // 由向右形状变换为向下形状
				if ((x - 2 >= 0 ) && y + 2 < gamePanel.yPointCount) {
					block[0] = blockPoint[x + 0][y + 2];
			...
			case 3:
			{
				   ...
				   // 由向左形状变换为向上形状
				if ((y - 2 >= 0 ) && (x + 2 < gamePanel.xPointCount)) {
					block[0] = blockPoint[x + 0][y - 2];
					block[1] = blockPoint[x + 0][y - 1];
					block[2] = blockPoint[x + 1][y - 1];
					block[3] = blockPoint[x + 2][y - 1];
