...
public class ShapeZig extends Shape implements InterfaceShapeMotion{
	...
	GamePanel gamePanel;
	public ShapeZig(GamePanel gamePanel) {
		...
		shapePositionPoint[2] = blockPoint[gamePanel.xPointCount / 2 - 2][1];
		shapePositionPoint[3] = blockPoint[gamePanel.xPointCount / 2 - 2][2];
	...
	public void changeShape() {
		...
		switch (shape) {
			...
			case 0:
			{   // 由向上形状变换为向右形状
				if(y + 2 < gamePanel.yPointCount
				   && x - 1 >= 0 && x + 1 < gamePanel.xPointCount) {   // 判断形状变换后是否超出游戏区域边界
					
					block[0] = blockPoint[x - 1][y + 1];
					block[1] = blockPoint[x + 0][y + 1];
					block[2] = blockPoint[x + 0][y + 2];
			...
			case 1:
			{   // 由向右形状变换为向上形状
				if ((y - 1 >= 0 ) && (y + 1 < gamePanel.yPointCount)
					&& (x + 1 < gamePanel.xPointCount)) {
					block[0] = blockPoint[x + 1][y - 1];
					block[1] = blockPoint[x + 1][y + 0];
					block[2] = blockPoint[x + 0][y + 0];
					block[3] = blockPoint[x + 0][y + 1];
				} else {
					break;
				}
